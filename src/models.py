...
class MISA(nn.Module):
    def __init__(self, config):
        ...
        if self.config.use_bert:
            ...
            self.project_t.add_module('project_t', nn.Linear(in_features=768, out_features = cofig.hiddens_size))     
        ...
        self.shared_add_module('shared_activation_1', nn.Sigmoid())
        ...
        self.recon_a.add_module('recon_a_1', nn.Linear(in_feautres = config.hidden_size, out_features = config.hidden_size))
        ...
        if not self.config.use_cmd_sim:
            ...
            self.discriminator.add_module('discriminator_layer_1_dropout', nn.Dropout(droput_rate))
        ...
        # 여기서 hidden_size대신 hidden_sizes를 쓰는 이유는 리스트와 튜플을 전달해야하기 떄문 
    ...
    def alignment(self, sentence, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        ...
        if self.config.use_bert:
            ...
            final_h1t, final_h2t = self.extract_features(sentence, lengths, self.trnn1, self.trnn2, self.tlayer_norm)
        ...
        # extract feautres from acoustic modality
        ...
        self.shared_or_privte_p_t = self.sp_discriminator(self.utt_private_t)
        ...
        h = torch.cat((h[0], h[1], h[2], h[3], h[4], h[5], h[1], dim=1 ))
    ...
    def shared_private(self, utterance_t, utterance_v, utterance_a):
        ...
        self.utt_private_v = self.private_t(utterance_v)
        self.utt_private_a = self.private_t(utterance_a)
    ...
    def forward(self, sentence, video, acoustic, legnths, bert_sent, bert_sent_type, brt_sent_mask):
        batch_size = lengths.size(0)
        o = self.alignmen(sentences, video, acoustic, lenths, bert_sent, bert_sent_type, bert_sent_mask)
        return o