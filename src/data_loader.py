...
class MSADataset(Dataset):
    ...
    def get_loader(config, shuffle=True):
        ...
        def collate_fn(batch):
            ...
            sentences = pad_sequences([torch.LongTensor(sample[0][0])for sample in batch], padding_value = PAD)
            ...
            # Create bert indicies using tokenizer
            ...
            for sample in batch:
                ...
                bert_details.append(encpded_bert_sent)
                
                
            # Bert things are batch_first 
            bert_sentences = torch.LongTensor([sample["input_ids"] for sample in bert_details])
            bert_sentence_types = torch.LongTensor([sample["token_type_ids"] for sample in bert_details])
            bert_setence_att_mask = torch.LongTensor([sample["attention_mask"] for sample in bert_details])
